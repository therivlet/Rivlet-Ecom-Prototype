    ...
    ${
      order
        ? `<div class="order-pill">${order.orderId}</div>
           <p style="color:var(--color-ink-muted)">Confirmation preview for <strong>${order.email}</strong> · ${formatPrice(order.total)}</p>`
        : `<p style="color:var(--color-ink-muted)">No recent prototype order found.</p>`
    }
